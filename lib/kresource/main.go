...
func GetTotalPodRequest(labels map[string]string, limitOrRequest string) (*Res, error) {
	...
	out, err := functions.ExecCmd(fmt.Sprintf(`kubectl get pods -A -o jsonpath-as-json={.items[*].spec.containers[*].resources.%s}`, limitOrRequest)+l, "", true)
	if err != nil {
		return nil, err
	}
...
func GetTotalResource(labels map[string]string) (*Res, error) {
	...
	out, err := functions.ExecCmd(
		`kubectl get nodes -o jsonpath-as-json={.items[*].status.allocatable}`+l, "", true)
